...
for root, dirs, files in os.walk(base_dir):
    ...
    for f in files:
        ...
        # ---- MetaLigand ----
            ...
            # NPLRdb_human.csv is the base: L -> Metabolite_Name, R -> Receptor_Gene_Symbol.
            # Ligand_list.csv is annotation only — loaded here and merged in; skip its own loop iteration.
            if f == 'NPLRdb_human.csv':
                df = df.rename(columns={'L': 'Metabolite_Name', 'R': 'Receptor_Gene_Symbol'})
                ligand_list_path = os.path.join(root, 'Ligand_list.csv')
                if os.path.exists(ligand_list_path):
                    ll = pd.read_csv(ligand_list_path)
                    ll = ll.rename(columns={'Compounds': 'Metabolite_Name', 'Transporter_genes': 'Transporter'})
                    ll = ll.drop(columns=[c for c in ['Receptor_genes', 'Receptor genes'] if c in ll.columns])
                    # Only bring in columns not already in the base
                    cols_to_merge = [c for c in ll.columns if c == 'Metabolite_Name' or c not in df.columns]
                    ll_sub = ll[cols_to_merge]
                    dedup_on = ['Metabolite_Name'] if 'Metabolite_Name' in ll_sub.columns else None
                    ll_dedup = ll_sub.drop_duplicates(subset=dedup_on) if dedup_on else ll_sub.drop_duplicates()
                    df = df.merge(ll_dedup, on='Metabolite_Name', how='left')
                df = df.dropna(axis=1, how='all')
                df['database'] = db_name
                is_metab = True
                
            elif f == 'Ligand_list.csv':
                continue  # handled as annotation inside the NPLRdb_human.csv branch above
            else:
                is_metab = True
